...
- **Financing structure** (supports multiple loans):
...
Common for investors who use gap funding, private money, or a second lien to cover the difference between the first mortgage and total project cost. Only include if user specifies.
...
**Financing cost formula (per loan position):**
```
Monthly Interest = Loan Amount × (Annual Rate / 12)
Points Cost = Loan Amount × Points %
...
Example (single hard money loan): $202,000 × (12% / 12) = $2,020/month × 4 months + $0 points = $8,080
...
| Staging | Optional — only for full rehab flips | $0-$2,500 |
| CAT Tax | Capital Acquisition Tax (where applicable) | Varies by jurisdiction |
| Selling title insurance | ~0.64% of sale price | Included above |
...
Net Profit = ARV
           - Purchase Price
           - Rehab Cost
           - Total Financing (all loans)
           - Holding Costs
           - Buying Costs
           - Selling Costs

Total Costs ROI = Net Profit / (Purchase + Rehab + Financing + Holding + Buying + Selling)
```

### Capital Analysis & Advanced ROI Metrics
...
This is the full picture that matters. A deal that looks like $46K profit on simple math might actually be $31K after all costs — that's a 32% reduction. Getting this wrong means making bad buying decisions.
...
```json
{
  ...
  "deal_analysis": {
    ...
    "rehab_purchase_rehab_roi_pct": 0.0,
    "wholetail_purchase_rehab_roi_pct": 0.0,
    "capital_analysis": {
      "rehab_down_payment": 0,
      "rehab_committed_capital": 0,
      "rehab_cash_on_cash_return": 0.0,
      "rehab_annualized_cocr": 0.0,
      "wholetail_down_payment": 0,
      "wholetail_committed_capital": 0,
      "wholetail_cash_on_cash_return": 0.0,
      "wholetail_annualized_cocr": 0.0
    },
    "rehab_profit_per_month": 0,
    "wholetail_profit_per_month": 0,
    "rehab_cost_per_sf": 0.0,
    "wholetail_cost_per_sf": 0.0,
