...
def client():
    ...
    if standalone is not None:
        log.info("using the standalone reisift client (%s auth)", standalone._scheme)
        return standalone
    _UNAVAILABLE = (
        _UNAVAILABLE
        or "no Deal Room clients, no REISIFT_API_KEY, and no DataSift token; "
        "CRM writes are unavailable"
    )
    return None
