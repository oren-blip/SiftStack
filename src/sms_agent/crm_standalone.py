...
class StandaloneCRM:
    ...
    def __init__(
        ...
        """`scheme` is the authorization prefix: "Api-Key" for the no-expiry Open
        API key, "Bearer" for the UI's rs_token on accounts without one.

        `refresh` is called once on a 401 to mint a replacement credential. It
        is what makes the Bearer path survivable unattended: an Api-Key never
        expires, a JWT does, and without this the worker would simply go quiet.
        """
        if not api_key:
    ...
    def _headers(self, content_type: Optional[str] = "application/json") -> dict:
        headers = {
            "authorization": f"{self._scheme} {self._key}",
        ...
        *,
        ...
        for attempt in range(4):
            ...
            if resp.status_code == 401 and self._refresh and not refreshed:
                # A Bearer token expired mid-run. Mint one and retry ONCE: a
                # refresh loop against a genuinely rejected credential would
                # relaunch a browser on every request.
                refreshed = True
                new_key = ""
                try:
                    new_key = self._refresh() or ""
                except Exception as exc:  # noqa: BLE001
                    log.warning("token refresh raised: %s", exc)
                if new_key and new_key != self._key:
                    log.info("reisift 401; refreshed the token and retrying")
                    self._key = new_key
                    continue
                raise StandaloneCRMError(
                    f"HTTP 401 on {method} {path} and the token could not be refreshed"
                )
...
def from_env() -> Optional[StandaloneCRM]:
    """Build a client, preferring the Open API key and falling back to Bearer.

    Order matters. REISIFT_API_KEY never expires and costs nothing to use, so it
    wins whenever it is present. Accounts outside the Open API beta have no such
    key; they authenticate with the UI's rs_token exactly as the rest of
    SiftStack already does, and pay for it with a refresh path.
    """
    ...
    from . import siftstack_auth

    tok = siftstack_auth.token()
    if not tok:
        return None
    try:
        return StandaloneCRM(tok, scheme="Bearer", refresh=siftstack_auth.refresh)
    except StandaloneCRMError as exc:
        log.warning("standalone CRM unavailable: %s", exc)
        return None
...
def check() -> tuple[bool, str]:
    """Probe the credential without writing anything."""
    client = from_env()
    if client is None:
        return False, "no REISIFT_API_KEY and no DataSift token could be obtained"
    try:
        client._request("/api/internal/property/", params={"limit": 1})
    except StandaloneCRMError as exc:
        return False, str(exc)[:200]
    return True, f"{client._scheme} accepted"
